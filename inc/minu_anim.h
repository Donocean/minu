...
typedef struct
{
    ...
    uint16_t duration;                   /* animation's duration (unit: ms), must smaller than maxT */
...
static inline void minu_anim_setTarget(minu_anim_t *me, int16_t start, int16_t end)
{
    me->start = start;
    me->end   = end;
}
...
static inline void minu_anim_setVarCb(minu_anim_t *me, void *var, void (*cb)(void *, int16_t))
{
    me->var = var;
    me->exec_cb = cb;
}
static inline void minu_anim_setConfig(minu_anim_t *me,
                                       uint32_t     start,
                                       uint32_t     end,
                                       uint32_t     duration,
                                       easingPath   path)
